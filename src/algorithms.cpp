...
lb::intervals lb::get_intervals(const lb::interval& start, const lb::alphabet& a, const lb::wtree& wt) {
    ...
    work_queue.push(tmp_node(0, start, {0, a.size() - 1}));
    ...
    while (!work_queue.empty()) {
        ...
        if (alpha.first == alpha.second) {
            ...
            list.emplace_back(C + range.first, C + range.second);
        } else {
            ...
            if (b0 > a0)
                work_queue.push(tmp_node(2*node + 1, {a0, b0 - 1}, {alpha.first, m}));
            if (b1 > a1)
                work_queue.push(tmp_node(2*node + 2, {a1, b1 - 1}, {m + 1, alpha.second}));
        }
    }
...
//        std::cout << "Intervals for: " << curr.first << "\n";
        auto list = get_intervals(curr.first, a, wt);
//        std::cout << list;
        for (const auto &intvl : list) {
